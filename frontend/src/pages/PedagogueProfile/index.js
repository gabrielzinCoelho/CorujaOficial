...
export default class PedagogueProfile extends Component {
  ...
  handleUpdate = async () => {
    ...
    if(this.state.previewFile){
      ...
      sessionStorage.setItem('path', dataFile.data.path);
    ...
    if(this.state.previewFile){
      ...
      const dataFileRemoved = await api.delete(`/file/${this.state.pedagogue.file_id}`, {
        headers: {
          "Authorization": `Bearer ${token}`
        }
      })
                  ...
                  <div className="profile-img">
                    {(() => {
                      ...
                      const path = this.state.previewFile ? this.state.previewFile : (
                        require(`../../../../backend/app/uploads/${this.state.pedagogue.path}`)
                      )
